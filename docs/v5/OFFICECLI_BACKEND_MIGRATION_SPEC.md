...
- 首版发布必过平台覆盖 Windows x64 与 Apple Silicon macOS；锁文件保留 OfficeCLI v1.0.113 的 8 个官方资产用于识别、缓存和后续扩展，其他 runtime 为 best-effort/后补兼容。
...
V5-014 发布必过平台限定为 `win-x64`、`osx-arm64`，对应常见 Windows 与 Apple Silicon macOS 桌面 Codex CLI 使用场景。锁文件仍保留 OfficeCLI v1.0.113 的 8 个官方资产，用于 runtime resolver 识别、离线缓存和未来扩展；`osx-x64`、`win-arm64` 与 Linux 系列证据属于 best-effort/后补兼容，不作为 V5-014/V5-015 发布阻塞项。

平台证据必须包含 runner 信息、资产 hash、`--version`、capability hash、最小 DOCX create/get/set/validate/screenshot 结果。

发布候选 evidence root 只应放入发布必过 runtime 的平台证据。best-effort 证据若被放入同一发布 evidence root，文件格式损坏或未知 runtime 仍视为证据集合污染；best-effort 证据应单独归档，不参与 V5-014/V5-015 阻塞 Gate。

Word/WPS TOC 测试优先使用专用 Windows runner 自动生成证据。若 GitHub self-hosted runner 暂不可用，允许维护者在本机 Windows + Word/WPS 环境生成 Word 与 WPS 两份 native TOC evidence，并通过 `scripts/officecli/v5_release_gate.py native-toc` 校验后作为发布候选证据；专用 runner 仍保留为后补自动化路径，不得删除。
...
- [ ] native TOC 未经 Word/WPS 精确刷新不能 accepted；CI dedicated runner 或本机维护者证据均必须通过 `native-toc` Gate。
- [ ] win/mac 必过 runtime（`win-x64`、`osx-arm64`）均有通过证据；其他已知 runtime 证据为 best-effort，不阻塞发布。
- [ ] 所有 L1 Gate 自动化测试通过。
- [ ] v4 历史 run 不被静默执行。
...
成功条件：COM 调用无异常、输出 hash 改变或 TOC 已确认无需字节变化、TOC entry 非空、页码均为正整数或合法范围、二次 reopen 成功、OfficeCLI validate clean、页数与截图证据一致。Word/WPS 各自必须有真实 Windows 原生刷新证据；专用 Windows runner 是优先与后补自动化路径，维护者本机证据必须通过 `native-toc` Gate 后才能替代 runner 证据。WPS 不可用不能由 Word 测试替代发布要求。
