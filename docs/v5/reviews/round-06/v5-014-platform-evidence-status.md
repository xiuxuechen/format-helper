...
本机已在同一候选提交 `1d801d49ab59d71070e3db68bd3472068e0c20ae` 上补跑 Word/WPS native TOC evidence smoke，Word 与 WPS 均为 `passed`。该本机证据用于证明代码路径可执行，但不替代 CI dedicated runner Gate；正式 V5-015 发布切换仍需 GitHub Actions 中 `officecli-windows-word` 与 `officecli-windows-wps` runner 上传证据。
...
|---|---|---|
| 无 | 无 | 必过平台证据已由 run `28559656986` 生成并通过聚合 Gate；后续等待 native-toc-evidence dedicated runner 上传正式证据。 |

## 下一步

1. 等待 run `28559656986` 的 `native-toc-evidence (word, officecli-windows-word)` 接单并上传 Word native TOC evidence。
2. 等待 run `28559656986` 的 `native-toc-evidence (wps, officecli-windows-wps)` 接单并上传 WPS native TOC evidence。
