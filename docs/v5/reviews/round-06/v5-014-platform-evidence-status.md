...
最新候选 GitHub Actions run `28761474155` 已通过 `platform-contract-validation`、`win-x64` evidence、`osx-arm64` evidence 与 `aggregate-platform-gate`。由于 Gate 已收敛，当前平台 evidence 不再阻塞 V5-014；不同 head_sha 的 PASS 仍不得混合作为同一发布候选。

本机已在候选分支上补跑 Word/WPS native TOC evidence，Word 与 WPS 均为 `passed`。该本机证据经 `scripts/officecli/v5_release_gate.py native-toc` 校验后可作为 V5-014/V5-015 native TOC 发布候选证据；GitHub Actions 中 `officecli-windows-word` 与 `officecli-windows-wps` runner 仍保留为后补自动化路径。
...
| runtime_id | 状态 | 证据路径 | 说明 |
|---|---|---|---|
| `win-x64` | passed | `officecli-win-x64-evidence` | GitHub Actions run `28761474155` 上传，artifact digest `sha256:106089cae4d39df91c27f04851b4d90036c570a600a6e88c86fc25a72da7f3f0`。 |
| `osx-arm64` | passed | `officecli-osx-arm64-evidence` | GitHub Actions run `28761474155` 上传，artifact digest `sha256:733bee290161b0c18c20226ed557374cacd44c6c8f2d6deda6dbc85d283523df`。 |
...
最新候选证据表中的 `win-x64` 与 `osx-arm64` digest 来自 GitHub Actions artifact 元数据，均绑定到 head_sha `86e325998fded8bfc16019f815fb446487fc714a`。
...
python -m unittest tests.validation.test_officecli_runtime_resolver tests.validation.test_officecli_platform_evidence tests.validation.test_officecli_native_toc_evidence tests.validation.test_toc_refresh_adapter_v5 tests.validation.test_officecli_v5_release_gate tests.validation.test_repair_plan_v5 -v
Ran 91 tests in 15.863s
OK
```
...
GitHub Actions run `28558124509` 上一候选状态：
...
| `native-toc-evidence (word, officecli-windows-word)` | queued | waiting runner，后补自动化 |
| `native-toc-evidence (wps, officecli-windows-wps)` | queued | waiting runner，后补自动化 |

`aggregate-platform-gate` 已成功下载并校验 `win-x64`、`osx-arm64` 两个必过 platform evidence artifact；Word/WPS native TOC dedicated runner 证据不再作为唯一发布放行来源，本机维护者证据通过 `native-toc` Gate 后可放行。

GitHub Actions run `28761474155` 当前状态：
...
| `platform-evidence (win-x64, windows-latest)` | completed | success |
| `aggregate-platform-gate` | completed | success |
| `native-toc-evidence` | completed | skipped，PR 触发时后补自动化路径不运行 |
| `aggregate-native-toc-gate` | completed | skipped，PR 触发时后补自动化路径不运行 |

本机 native TOC smoke 证据：

| viewer | 状态 | 证据路径 | 说明 |
|---|---|---|---|
| Microsoft Word 12.0 | passed | `artifacts/native-toc-evidence-local/word-final-3/logs/native_toc.platform-evidence.json` | 本机 COM + locked OfficeCLI v1.0.113，截图 sha256 `c4ab4600537546fcbc88fb4dc23177ded2aac53aebb7333d3b64c8d1ac8054e3`。 |
| WPS Writer 12.0 | passed | `artifacts/native-toc-evidence-local/wps-final-3/logs/native_toc.platform-evidence.json` | 本机 COM + locked OfficeCLI v1.0.113，截图 sha256 `c4ab4600537546fcbc88fb4dc23177ded2aac53aebb7333d3b64c8d1ac8054e3`。 |

本机证据已整理为干净发布候选目录 `artifacts/native-toc-evidence-release/`，并通过 native TOC Gate：

```text
python scripts/officecli/v5_release_gate.py native-toc --evidence-root artifacts/native-toc-evidence-release --lock tools/officecli/officecli.lock.json
{"ok": true, "errors": []}
```
...
|---|---|---|
| 无 | 无 | 必过平台证据已由 run `28761474155` 生成并通过聚合 Gate；native TOC 已由本机 Word/WPS evidence 覆盖且通过 `native-toc` Gate，dedicated runner 仅作为后补自动化证据。 |

## 下一步

1. V5-015 release readiness 已同步为“发布切换前复核通过，等待人工确认正式发布切换/合并”。
2. `officecli-windows-word` 与 `officecli-windows-wps` runner 可在上线后通过 `workflow_dispatch` + `run_native_toc_dedicated=true` 后补 CI 证据。
