...
最新候选 GitHub Actions run `28559656986` 已通过 `platform-contract-validation`、`win-x64` evidence、`osx-arm64` evidence 与 `aggregate-platform-gate`。由于 Gate 已收敛，当前平台 evidence 不再阻塞 V5-014；不同 head_sha 的 PASS 仍不得混合作为同一发布候选。
...
| runtime_id | 状态 | 证据路径 | 说明 |
|---|---|---|---|
| `win-x64` | passed | `officecli-win-x64-evidence` | GitHub Actions run `28559656986` 上传，artifact digest `sha256:d9306ec499ef0a7163c5bf748226c98fdbaf033d9d18b14f74baa66981dc5356`。 |
| `osx-arm64` | passed | `officecli-osx-arm64-evidence` | GitHub Actions run `28559656986` 上传，artifact digest `sha256:2d639664168b0996c925de9bbe19b5b8dd553b9c1c80a89c2c39f787fec23027`。 |
...
最新候选证据表中的 `win-x64` 与 `osx-arm64` digest 来自 GitHub Actions artifact 元数据，均绑定到 head_sha `1d801d49ab59d71070e3db68bd3472068e0c20ae`。
...
GitHub Actions run `28559656986` 当前状态：
...
|---|---|---|
| 无 | 无 | 必过平台证据已由 run `28559656986` 生成并通过聚合 Gate；native TOC 已由本机 Word/WPS evidence 覆盖且通过 `native-toc` Gate，dedicated runner 仅作为后补自动化证据。 |

## 下一步

1. 同步 V5-015 release readiness，把发布切换阻塞从 `blocked_waiting_runner` 改为“native TOC 本机证据已满足，dedicated runner 后补”。
