...
1. V5-014 win/mac 必过平台证据齐全：`win-x64`、`osx-x64`、`osx-arm64`。
...
| Gate | 当前状态 | 说明 |
|---|---|---|
| V5-014 platform-contract-validation | passed | run `28504129828` 已通过。 |
| V5-014 `win-x64` evidence | passed | run `28504129828` 已通过。 |
| V5-014 `osx-arm64` evidence | passed | run `28504129828` 已通过。 |
| V5-014 `osx-x64` evidence | blocked_waiting_runner | run `28504129828` 中 `platform-evidence (osx-x64, macos-13)` 仍 queued。 |
| aggregate-platform-gate | not_started | 等待三项 win/mac platform evidence 全部完成。 |
| native-toc-evidence | not_started | 等待 aggregate-platform-gate 后进入 Word/WPS dedicated runner。 |
...
| 平台 Gate 收敛 | ready | V5-014 已收敛到 `win-x64`、`osx-x64`、`osx-arm64` 必过。 |
| 发布切换执行 | blocked | 必须等待 `osx-x64`、aggregate-platform-gate、native-toc-evidence 全部通过。 |

## 放行条件

只有当以下状态全部满足时，才能将 V5-015 标为可发布切换：

- run `28504129828` 或后续同一 head_sha 候选 run 生成 `win-x64`、`osx-x64`、`osx-arm64` 三个平台证据。
- `aggregate-platform-gate` 成功校验三平台 evidence。
