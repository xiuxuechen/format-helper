...
| Gate | 当前状态 | 说明 |
|---|---|---|
| V5-014 platform-contract-validation | passed | run `28504451037` 已通过。 |
| V5-014 `win-x64` evidence | passed | run `28504451037` 已通过。 |
| V5-014 `osx-arm64` evidence | passed | run `28504451037` 已通过。 |
| aggregate-platform-gate | not_started | 等待两项 win/mac platform evidence 被聚合校验。 |
...
| 发布切换执行 | blocked | 必须等待 aggregate-platform-gate、native-toc-evidence 全部通过。 |

## 放行条件

只有当以下状态全部满足时，才能将 V5-015 标为可发布切换：

- run `28504451037` 或后续同一 head_sha 候选 run 生成 `win-x64`、`osx-arm64` 两个平台证据。
