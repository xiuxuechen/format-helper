...
| native-toc-evidence 本机 smoke | passed | 本机 Word 12.0 与 WPS Writer 12.0 均已通过 `artifacts/native-toc-evidence-local/*-final-3/logs/native_toc.platform-evidence.json`。该证据不替代 CI dedicated runner Gate。 |
| native-toc-evidence CI dedicated runner | blocked_waiting_runner | run `28559656986` 中 Word/WPS dedicated runner job 已进入 queued，等待 `officecli-windows-word` 与 `officecli-windows-wps` 接单。 |
...
| 本机 Word/WPS native TOC smoke | ready | Word 与 WPS 均已在本机通过，可作为 dedicated runner 排队期间的诊断证据。 |
