...
description: 【V5 DEPRECATED】v4 DOCX 写回技能，仅供历史 run 只读说明。v5 写回必须使用 scripts/officecli/request_builder.py 与 runtime_adapter.py。
---
...
仅在识别到历史 v4 写回调用时用于返回退役阻断说明；不得执行修复。

## 定位

本技能不再是可执行生产能力。v5 不允许调用本目录下的旧 Python OOXML 写回脚本。
