...
- 用户确认后，format-helper 主控读取 `plans/manual_review_items.json` 的 `decision`，生成新的 `role_format_slot_facts` revision，把对应槽位回填为 `status=user_confirmed`、`primary_source=user_confirmed`、`confidence=1.0`，再将 Gate 摘要状态更新为 `cleared`。
...
5. 对低置信度、高风险、缺失产物、未解析槽位或冲突槽位停止推进，输出人工确认项；建规确认必须同时写 `logs/rule_confirmation_gate.json` 与 `plans/manual_review_items.json`。
