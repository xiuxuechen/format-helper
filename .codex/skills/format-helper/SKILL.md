...
- 【V5】不直接写 Word；最终写回由 `scripts/officecli/runtime_adapter.py` 通过 OfficeCLI batch 执行。
...
- 【V5】`scripts/officecli/snapshot_adapter.py`：生成 OfficeCLI snapshot v2。
...
- 【V5】`scripts/officecli/runtime_adapter.py`：通过 OfficeCLI batch 执行修复（替代 docx-format-repairer）。
