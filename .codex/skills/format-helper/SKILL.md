...
- 【OFFICECLI】不直接写 Word；最终写回由 `scripts/officecli/runtime_adapter.py` 通过 OfficeCLI batch 执行。
...
- 【OFFICECLI】`scripts/officecli/snapshot_adapter.py`：生成 OfficeCLI snapshot v2。
...
- `docx-repair-planner`：生成 `repair_plan.finalized.r*.yaml`。
- 【OFFICECLI】`scripts/officecli/runtime_adapter.py`：通过 OfficeCLI batch 执行修复（替代 docx-format-repairer）。
...
- `contracts/officecli/schemas/`：OfficeCLI 运行契约 schema。
- `contracts/format-helper/schemas/`：format-helper 结构化产物 schema。
- `scripts/officecli/release_gate.py`：发布静态 Gate、平台 evidence Gate 与 native TOC Gate。
